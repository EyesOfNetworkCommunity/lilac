...
/**
 * Base class that represents a row from the 'nagios_host_contact_member' table.
 *
 * Contacts which belong to host templates or hosts
 *
 * @package    propel.generator..om
 */
abstract class BaseNagiosHostContactMember extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'NagiosHostContactMemberPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        NagiosHostContactMemberPeer
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = NagiosHostContactMemberPeer::NUM_HYDRATE_COLUMNS.
	...
	{
		if (isset($alreadyDumpedObjects['NagiosHostContactMember'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['NagiosHostContactMember'][$this->getPrimaryKey()] = true;
	...
	public function getNagiosHostTemplate(PropelPDO $con = null)
	{
		if ($this->aNagiosHostTemplate === null && ($this->template !== null)) {
			$this->aNagiosHostTemplate = NagiosHostTemplateQuery::create()->findPk($this->template, $con);
			/* The following can be used additionally to
				guarantee the related object contains a reference
				to this object.  This level of coupling may, however, be
				undesirable since it could result in an only partially populated collection
