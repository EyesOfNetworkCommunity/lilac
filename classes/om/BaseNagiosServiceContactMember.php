...
/**
 * Base class that represents a row from the 'nagios_service_contact_member' table.
 *
 * Contacts which belong to service templates or services
 *
 * @package    propel.generator..om
 */
abstract class BaseNagiosServiceContactMember extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'NagiosServiceContactMemberPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        NagiosServiceContactMemberPeer
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = NagiosServiceContactMemberPeer::NUM_HYDRATE_COLUMNS.
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(NagiosServiceContactMemberPeer::ID) ) {
	...
	{
		if (isset($alreadyDumpedObjects['NagiosServiceContactMember'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['NagiosServiceContactMember'][$this->getPrimaryKey()] = true;
	...
	{
		return (string) $this->exportTo(NagiosServiceContactMemberPeer::DEFAULT_STRING_FORMAT);
	}
