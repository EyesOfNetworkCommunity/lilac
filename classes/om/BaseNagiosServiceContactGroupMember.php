...
/**
 * Base class that represents a row from the 'nagios_service_contact_group_member' table.
 *
 * Nagios  Service Group
 *
 * @package    propel.generator..om
 */
abstract class BaseNagiosServiceContactGroupMember extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'NagiosServiceContactGroupMemberPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        NagiosServiceContactGroupMemberPeer
	 */
	...
	/**
	 * The value for the contact_group field.
	 * @var        int
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = NagiosServiceContactGroupMemberPeer::NUM_HYDRATE_COLUMNS.
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(NagiosServiceContactGroupMemberPeer::ID) ) {
	...
	{
		if (isset($alreadyDumpedObjects['NagiosServiceContactGroupMember'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['NagiosServiceContactGroupMember'][$this->getPrimaryKey()] = true;
	...
	public function getNagiosContactGroup(PropelPDO $con = null)
	{
		if ($this->aNagiosContactGroup === null && ($this->contact_group !== null)) {
			$this->aNagiosContactGroup = NagiosContactGroupQuery::create()->findPk($this->contact_group, $con);
			/* The following can be used additionally to
				guarantee the related object contains a reference
				to this object.  This level of coupling may, however, be
				undesirable since it could result in an only partially populated collection
	...
	{
		return (string) $this->exportTo(NagiosServiceContactGroupMemberPeer::DEFAULT_STRING_FORMAT);
	}
