...
abstract class BaseAutodiscoveryDeviceTemplateMatch extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
	const PEER = 'AutodiscoveryDeviceTemplateMatchPeer';
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 5; // 5 = AutodiscoveryDeviceTemplateMatchPeer::NUM_HYDRATE_COLUMNS.
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(AutodiscoveryDeviceTemplateMatchPeer::ID) ) {
	...
	{
		if (isset($alreadyDumpedObjects['AutodiscoveryDeviceTemplateMatch'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['AutodiscoveryDeviceTemplateMatch'][$this->getPrimaryKey()] = true;
	...
	{
		return (string) $this->exportTo(AutodiscoveryDeviceTemplateMatchPeer::DEFAULT_STRING_FORMAT);
	}
