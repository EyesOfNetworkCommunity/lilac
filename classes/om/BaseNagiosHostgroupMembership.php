...
/**
 * Base class that represents a row from the 'nagios_hostgroup_membership' table.
 *
 * Hostgroup Membership for Host
 *
 * @package    propel.generator..om
 */
abstract class BaseNagiosHostgroupMembership extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'NagiosHostgroupMembershipPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        NagiosHostgroupMembershipPeer
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = NagiosHostgroupMembershipPeer::NUM_HYDRATE_COLUMNS.
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(NagiosHostgroupMembershipPeer::ID) ) {
	...
	{
		if (isset($alreadyDumpedObjects['NagiosHostgroupMembership'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['NagiosHostgroupMembership'][$this->getPrimaryKey()] = true;
	...
	{
		return (string) $this->exportTo(NagiosHostgroupMembershipPeer::DEFAULT_STRING_FORMAT);
	}
