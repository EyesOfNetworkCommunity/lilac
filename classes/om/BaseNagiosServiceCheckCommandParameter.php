...
/**
 * Base class that represents a row from the 'nagios_service_check_command_parameter' table.
 *
 * Parameter for check command for service or service template
 *
 * @package    propel.generator..om
 */
abstract class BaseNagiosServiceCheckCommandParameter extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'NagiosServiceCheckCommandParameterPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        NagiosServiceCheckCommandParameterPeer
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = NagiosServiceCheckCommandParameterPeer::NUM_HYDRATE_COLUMNS.
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				NagiosServiceCheckCommandParameterQuery::create()
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(NagiosServiceCheckCommandParameterPeer::ID) ) {
	...
	{
		if (isset($alreadyDumpedObjects['NagiosServiceCheckCommandParameter'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['NagiosServiceCheckCommandParameter'][$this->getPrimaryKey()] = true;
	...
	{
		return (string) $this->exportTo(NagiosServiceCheckCommandParameterPeer::DEFAULT_STRING_FORMAT);
	}
