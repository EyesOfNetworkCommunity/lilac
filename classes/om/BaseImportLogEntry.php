...
/**
 * Base class that represents a row from the 'import_log_entry' table.
 *
 * Import Job Entry
 *
 * @package    propel.generator..om
 */
abstract class BaseImportLogEntry extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'ImportLogEntryPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        ImportLogEntryPeer
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 5; // 5 = ImportLogEntryPeer::NUM_HYDRATE_COLUMNS.
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(ImportLogEntryPeer::ID) ) {
	...
	{
		if (isset($alreadyDumpedObjects['ImportLogEntry'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['ImportLogEntry'][$this->getPrimaryKey()] = true;
		...
		if ($includeForeignObjects) {
			if (null !== $this->aImportJob) {
				$result['ImportJob'] = $this->aImportJob->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
			}
		}
		return $result;
	}
	...
	public function getImportJob(PropelPDO $con = null)
	{
		if ($this->aImportJob === null && ($this->job !== null)) {
			$this->aImportJob = ImportJobQuery::create()->findPk($this->job, $con);
	...
	{
		return (string) $this->exportTo(ImportLogEntryPeer::DEFAULT_STRING_FORMAT);
	}
