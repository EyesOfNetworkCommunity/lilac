...
/**
 * Base class that represents a row from the 'nagios_contact_group_member' table.
 *
 * Member of a Nagios Contact Group
 *
 * @package    propel.generator..om
 */
abstract class BaseNagiosContactGroupMember extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'NagiosContactGroupMemberPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        NagiosContactGroupMemberPeer
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 3; // 3 = NagiosContactGroupMemberPeer::NUM_HYDRATE_COLUMNS.
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(NagiosContactGroupMemberPeer::ID) ) {
	...
	{
		if (isset($alreadyDumpedObjects['NagiosContactGroupMember'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['NagiosContactGroupMember'][$this->getPrimaryKey()] = true;
	...
	{
		return (string) $this->exportTo(NagiosContactGroupMemberPeer::DEFAULT_STRING_FORMAT);
	}
